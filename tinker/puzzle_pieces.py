...
def draw_puzzle_piece():
    ...
    image[:0:width] = (0, 255, 0)  # Green in BGR format
...
def add_alpha_layer_to_image(image):
    #     https://stackoverflow.com/questions/39642721/adding-alpha-channel-to-rgb-array-using-numpy
    # rgba = np.concatenate((image, np.zeros((205, 54, 1))), axis=2)
    rgba = cv2.cvtColor(image, cv2.COLOR_RGB2RGBA)
    return rgba
    return np.dstack((image, np.zeros(image.shape)))
...
if __name__ == '__main__':
    ...
    # img = draw_puzzle_piece()
    # turn image into alpha
    img = image_to_alpha()

    from import_image import image
    # added_image = cv2.addWeighted(add_alpha_layer_to_image(image), 0.4, img, 0.1, 0)
